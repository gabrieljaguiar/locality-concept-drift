...
ht <- read.csv("../output/HT_ADWIN_multi_class_local_prune_growth_new_branch_ds_1_c_5_ca_5_f_10_1_1.csv")
aht <- read.csv("../output/AHT_ADWIN_multi_class_local_prune_growth_new_branch_ds_1_c_5_ca_5_f_10_1_1.csv")
htdd <- read.csv("../output/HTDD_ADWIN_multi_class_local_prune_growth_new_branch_ds_1_c_5_ca_5_f_10_1_1.csv")
...
g <- ggplot(df, aes(x=idx, y=accuracy)) +
  ...
  scale_y_continuous(limits=c(0.0,0.99)) +
  theme_bw() + ylab("Accuracy") + xlab("Instances") +
  labs(title="Multi-Class Local", color="Classifier") +
...
ggsave("accuracy_multi_class_local_line.pdf", g, width = 4.5, height=4.5)
